...
module("plugin_server", package.seeall)
...
registerplugin(package.loaded.plugin_server)
-- End required code
...
-- Your handler function can optionally return true to block the chat (or false/nil if you do not want to block the chat). If you do this, no handlers past yours will get the chat message.
SE_CHAT = 15
...
SE_NAMECHANGE = 19
--- A constant for the event emitted when a player sends a command. This only occurs for things sent by the client, not what we fake from the client. 
-- player, the player who sent the command
-- command, an array with the command data
-- Your handler function can optionally return true to block this command.
SE_CLIENTCOMMAND = 20
--- A constant for the event emitted when a player has successfully connected server (is not banned/dropped).
-- player, the player who has connected
SE_CONNECT = 21
...
_G.server = package.loaded.plugin_server
...
function removeplayerbyent(ent) 
    ...
    debug("removeplayerbyent: taking place (checking for players never getting rm'd) for " .. uid .. " (this is "..tostring(players[uid].name)..")")
    ...
    if(ent == eventplayerent) then
        debug("removeplayerbyent: player is the event player!")
        eventplayerent = nil
        ...
        if(eventplayerent == nil) then
            debug("removeplayerbyent: never found a new event player to use!")
            -- GE_CLIENTBEGIN now handles a nil "eventplayerent", it should be the best way to handle things... so ignore it
            
            if(1==2) then -- TODO: RM THIS IF SURE
    ...
    debug("removeplayerbyent: before nilling player, table size is " .. extra.tablesize(players))
    dump.printtable(players, 4, 2)
    players[uid] = nil
    playercount = playercount - 1
    debug("removeplayerbyent: after nilling player, table size is " .. extra.tablesize(players))
end
...
-- This is optimized for use when trying to get EVENT_'s from the game.
...
--- Broadcasts a message to all players on the server, and also to the server console.
...
--- Spoofs a command as if it came from a player. This will make use of the
-- client command queue, and it is the preferred way to send fake client
-- commands.
-- @param player the player to fake this command from
-- @param command an array with the command (example: {"say", "hello guys!"}
-- @param handler an optional function to call just before and just after we tell the gamelib about this command (takes a boolean that is false if it's before sent, true if after sent)
function spoofclientcommand(player, command, handler)
    assert(player ~= nil, "player cannot be nil")
    assert(command ~= nil, "command cannot be nil")
    net_enqueueclientcommand(player, command, handler, false) -- don't intercept what we send
end
...
function spoofcommandargs(arraytable)
    assert(arraytable ~= nil, "arraytable cannot be nil")
    if(spoofing_commandargs) then
        ...
        for i=1,#arraytable do
            ...
            if(i >= 2) then 
                args_string = args_string .. arraytable[i] 
...
-- message queues. It helps us exit/skip over GE_RUNFRAME early if we know that
-- no other players have messages left.
net_remainingmessages = 0
--- This is similar to net_remainingmessages/
net_remainingclientcommands = 0
...
--       cmdqueue - A queue of client commands, where each entry has fields "command" (an array), "postwaitframes", "prewaitframes", "handler" (function), "interceptable"
--       waitframes - The number of frames to wait before processing the next command
...
--- Puts a client command in the command queue for a player. Please don't use 
-- this unless you are doing something very specialized.
-- @param player the player reference
-- @param command an array of the client's command
-- @param handler an optional function to call just before and just after we tell the gamelib about this command (takes a boolean that is false if it's before sent, true if after sent)
-- @param canintercept true if we want to allow SE_CLIENTCOMMAND to intercept this, false if we want it to remain invisible
function net_enqueueclientcommand(player, command, handler, canintercept)
    assert(player ~= nil, "player cannot be nil")
    assert(command ~= nill, "command cannot be nil")
    local exempt = false -- will be set to true if this command is not allowed in the queue
    if(#command > 0) then
        for i,commandname in ipairs(config.NET_CMDQUEUE_EXEMPT) do
            if(commandname:lower() == command[1]) then
                exempt = true
                break
            end
        end
    ...
    if(exempt) then
        ...
        if(handler ~= nil) then handler(false) end
        local finished = spoofcommandargs(command)
        ge.ClientCommand(player.ent)
        finished()
        if(handler ~= nil) then handler(true) end
    else
        ...
        if(#command > 0) then 
            ...
            entry.postwaitframes = config.NET_CMDQUEUE_FORCEWAIT[cmd]
        end
        if(entry.postwaitframes == nil) then entry.postwaitframes = 0 end
        entry.command = command
        entry.handler = handler
        entry.prewaitframes = 0 -- not used yet
        entry.interceptable = canintercept
        net_remainingclientcommands = net_remainingclientcommands + 1
        table.insert(netsettings.cmdqueue, 1, entry)
    end
end
...
local function gbl_processgamelibquery(player, command)
    ...
    local replyhandler = function(pwhash)
        ...
        spoofclientcommand(player, {GAMECMD_GLOGIN, pwhash})
    end
    -- Queue the query
    gbl_queueclientquery(player, dispatchhandler, replyhandler) 
end
...
local function gbl_localidupdate()
    ...
    -- Note: Now uses the queue system since sending players about 1-2 frames before a chat had DP ignore the chat ("say")...a bug in DP?
    spoofclientcommand(targetplayer, {"players"}, handler)  
end
...
listen.game.postcall(GE_CLIENTCONNECT, function(data) 
    ...
    if(testp ~= nil) then 
        -- TODO/WARN: I think there may be a situation where an old player is never removed, causing problems
        -- when we re-use the old player. p!nk says her name changed from another player's name (maybe...).
        -- The teambalance plugin may be getting wrong teams this way???
        debug("PHANTOMCHECK: CONNECT IGNORE; ALREADY HAVE " .. tostring(testp.name) .. " [userinfo: "..tostring(data.userinfo).."]")
    ...
    if(player.info.ip ~= nil) then
        ...
        debug("GAMELIB DROPPING CLIENT... problem spot here?")
    end
    ...
    emitevent(SE_CONNECT, function(f)
        f(player)
    end)
...
listen.game.postcall(GE_CLIENTDISCONNECT, function(data)
    -- TODO/WARN: I found a bug in here sep 8th; players must not be getting removed because PHANTOMCHECK occurs way too often, it's like GE_CLIENTDISCONNECTs are not being processed... let's see if my debug info added to the player removal func helps 
...
listen.game.precall(GE_SPAWNENTITIES, function(data)    
    ...
    -- Reset the player ent used for catching EVENT_'s, since we need the first person who enters the game, so that we ensure we get both theirs and everyone else's joins
    eventplayerent = nil
    
    -- Update players for this map change
    for i,player in pairs(players) do
        player.ingame = false
        player.alive = false
        player.flags, player.grabs, player.holds, player.caps, player.kills, player.deaths, player.streak = 0,0,0,0,0,0,0
        player.teamindex = nil -- Force things that rely on teamindex to implode if it never gets updated :)
    end
        
    -- Send out the event
    local mapname = util.deref_pp_char(data.mapname)
    emitevent(SE_MAPCHANGE, function(f) 
        f(mapname)
    end)
end)
...
listen.dp.postcall(GIDP_CPRINTF, function(data) 
    ...
    elseif(data.printlevel == PRINT_EVENT and data.ent ~= nil) then
        local player = getplayerbyent(data.ent)
        if(player ~= nil and not player.discarded and eventplayerent == nil) then -- Note: disconnecting player gets their own disconnect, then the player gets removed in post-CLIENTDISCONNECT, so we should be fine
            -- This player should start getting events we need (the server had no players before this one,
            -- or the eventplayerent was reset due to a map change, so we need the next "first player getting events in the game")
            eventplayerent = data.ent
        end        
        if(hashplayerent(data.ent) == hashplayerent(getarbitraryplayerent())) then
            ...
            if(eventid == qshared.EVENT_ENTER) then
                -- TODO: playerbalance saw a player on a nonexistent team, it must have carried from prev. map -- I think that the monitoring client never saw that player enter... maybe we need to change the monitoring client after a map change?
                ...
                debug("SE_ENTER   TEAMDEBUG ******", p.name, teamindex, getteamname(teamindex), "...", teamnum, teamindex)
            ...
            elseif(eventid == qshared.EVENT_JOIN) then
                ...
                debug("SE_JOINTEAM TEAMDEBUG", p.name, p.teamindex, getteamname(p.teamindex))
...
-- Initializes the client command dequeue
-- Capture commands
listen.game.precall(GE_CLIENTCOMMAND, function(data)
    -- Grab the args and then immediately block it
    local command = getcommandargs()
    -- Allow interception
    net_enqueueclientcommand(getplayerbyent(util.deref_pp_edict_t(data.ent)), command, nil, true)
    return intercept.DO_DROP
end)
-- Dispatch commands
listen.game.postcall(GE_RUNFRAME, function()
     if(net_remainingclientcommands > 0) then
        for i,player in pairs(players) do
            if(net_remainingclientcommands == 0) then
                return -- No reason to continue, no more players have commands
            end
            
            local netsettings = net_getplayertable(player)
            if(#netsettings.cmdqueue > 0) then
                if(netsettings.waitframes > 0) then
                    -- We need to wait before we do this command (part of a post-wait)
                    netsettings.waitframes = netsettings.waitframes - 1
                else
                    local entry = netsettings.cmdqueue[#netsettings.cmdqueue]
                    if(entry.prewaitframes > 0) then
                        -- We need to leave the entry here and just wait a frame
                        entry.prewaitframes = entry.prewaitframes - 1
                    else
                        -- No more frames to wait, process this entry
                        netsettings.cmdqueue[#netsettings.cmdqueue] = nil                        
                        net_remainingclientcommands = net_remainingclientcommands - 1 
                        
                        local blockcommand = false
                        if(entry.interceptable) then -- We can run it through our client command handler
                            emitevent(SE_CLIENTCOMMAND, function(f) 
                                blockcommand = blockcommand or f(player, entry.command)
                            end)
                        end
                        if(not blockcommand) then
                            local finishedspoofing = spoofcommandargs(entry.command)                  
                            if(entry.handler ~= nil) then entry.handler(false) end
                            ge.ClientCommand(player.ent) -- Tell the gamelib
                            if(entry.handler ~= nil) then entry.handler(true) end
                            finishedspoofing()
                            netsettings.waitframes = entry.postwaitframes -- Use whatever post-wait there is
                            -- Get rid of our previous chat listener (any CPRINTF for chat takes place between pre/post, and this listener is set up in our SE_CLIENTCOMMAND handler which is "pre")
...
addeventlistener(package.loaded.plugin_server, SE_MAPCHANGE, function()
    print("Performing Lua garbage collection for map change")
    collectgarbage("collect")
end)

-- Intercept regular chat and console chat
addeventlistener(package.loaded.plugin_server, SE_CLIENTCOMMAND, function(player, commands)
    debug("SE_CLIENTCOMMAND:", player.name, "->", unpack(commands))
    local gotchat = false
    ...
    if(gidp.argc() >= 2) then
        ...
        if(cmd == "SAY" or cmd == "SAY_TEAM" or cmd == "MESSAGEMODE" or cmd == "MESSAGEMODE2") then
            ...
            if(text ~= "") then -- Ignore empty messages, since the gamelib ignores them too
                emitevent(SE_CHAT, function(f) -- Send the event out
                    if(not blockchat) then -- Don't continue sending the event if it's blocked
    ...
    if(not gotchat and player ~= nil) then
        ...
        chatprintflistener = function(chatdata) -- global func, needs to be nil'd after gi.clientcommand
            ...
            if(ent == nil) then -- broadcast (players don't actually see this, they get individual encoded messages, like we generate GE_ events off of)
                ...
                if(string.match(text, "^.-" .. escapepatterntext(regularmatch) .. "$")) then
                    ...
                    if(blockchat) then 
                        dropallcprintf = true -- Block all messages after this one too (they are going to individual clients)
                        return intercept.DO_DROP 
